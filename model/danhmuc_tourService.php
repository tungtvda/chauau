...
{
   return danhmuc_tour_Get("SELECT danhmuc_tour.id, danhmuc_tour.name, danhmuc_tour.name_url, danhmuc_tour.img, danhmuc_tour.position, danhmuc_tour.title, danhmuc_tour.keyword, danhmuc_tour.description FROM  danhmuc_tour ".(($where!='')?(' where '.$where):'')." Order By ".$Order." Limit ".(($CurrentPage-1)*$PageSize)." , ".$PageSize);
}
//
function danhmuc_tour_insert($obj)
{
    return exe_query("insert into danhmuc_tour (name,name_url,img,position,title,keyword,description) values ('$obj->name','$obj->name_url','$obj->img','$obj->position','$obj->title','$obj->keyword','$obj->description')",'danhmuc_tour');
}
//
function danhmuc_tour_update($obj)
{
    return exe_query("update danhmuc_tour set name='$obj->name',name_url='$obj->name_url',img='$obj->img',position='$obj->position',title='$obj->title',keyword='$obj->keyword',description='$obj->description' where id=$obj->id",'danhmuc_tour');
}
//
