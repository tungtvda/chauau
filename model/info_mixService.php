...
{
   return info_mix_Get("SELECT info_mix.id, info_mix.img, info_mix.name, info_mix.content, info_mix.title, info_mix.keyword, info_mix.description FROM  info_mix ".(($where!='')?(' where '.$where):'')." Order By ".$Order." Limit ".(($CurrentPage-1)*$PageSize)." , ".$PageSize);
}
//
function info_mix_insert($obj)
{
    return exe_query("insert into info_mix (img,name,content,title,keyword,description) values ('$obj->img','$obj->name','$obj->content','$obj->title','$obj->keyword','$obj->description')",'info_mix');
}
//
function info_mix_update($obj)
{
    return exe_query("update info_mix set img='$obj->img',name='$obj->name',content='$obj->content',title='$obj->title',keyword='$obj->keyword',description='$obj->description' where id=$obj->id",'info_mix');
}
//
