...
{
   return news_Get("SELECT news.id, danhmuc_tour.name as danhmuc_id, news.name, news.name_url, news.img, news.view, news.content, news.title, news.keyword, news.description, news.created FROM  news, danhmuc_tour where danhmuc_tour.id=news.danhmuc_id  ".(($where!='')?(' and '.$where):'')." Order By ".$Order." Limit ".(($CurrentPage-1)*$PageSize)." , ".$PageSize);
}
//
