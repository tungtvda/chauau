...
{
   return video_Get("SELECT video.id, danhmuc_tour.name as danhmuc_id, video.name, video.img, video.link_video, video.highlights FROM  video, danhmuc_tour where danhmuc_tour.id=video.danhmuc_id  ".(($where!='')?(' and '.$where):'')." Order By ".$Order." Limit ".(($CurrentPage-1)*$PageSize)." , ".$PageSize);
}
//
function video_insert($obj)
{
    return exe_query("insert into video (danhmuc_id,name,img,link_video,highlights) values ('$obj->danhmuc_id','$obj->name','$obj->img','$obj->link_video','$obj->highlights')",'video');
}
//
function video_update($obj)
{
    return exe_query("update video set danhmuc_id='$obj->danhmuc_id',name='$obj->name',img='$obj->img',link_video='$obj->link_video',highlights='$obj->highlights' where id=$obj->id",'video');
}
//
